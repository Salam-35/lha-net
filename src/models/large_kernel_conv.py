...
class ChannelShuffle3D(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Check if channels is divisible by groups
        if channels % self.groups != 0:
            # Handle non-divisible case: shuffle only the divisible part
            channels_per_group = channels // self.groups
            shuffleable_channels = channels_per_group * self.groups

            # Split into shuffleable and remainder
            x_shuffle = x[:, :shuffleable_channels, :, :, :]
            x_remainder = x[:, shuffleable_channels:, :, :, :]

            # Shuffle the divisible part
            x_shuffle = x_shuffle.view(batch, self.groups, channels_per_group, d, h, w)
            x_shuffle = x_shuffle.transpose(1, 2).contiguous()
            x_shuffle = x_shuffle.view(batch, shuffleable_channels, d, h, w)

            # Concatenate back
            x = torch.cat([x_shuffle, x_remainder], dim=1)
        else:
            channels_per_group = channels // self.groups
